...
public class AppController {
	...
	@SuppressWarnings("unused")
	@GetMapping("/currencyCell")
	public String showCurrencyCell(@ModelAttribute("detail") Detail detail, Model model) {
		Detail newCell = new Detail();
		model.addAttribute("detail", newCell);
		List<account> listAccount = accountRepository.findAll();
		List<String> newlistAccountName = new ArrayList<String>();
		List<Float> listBalance = new ArrayList<Float>();
		for (int i = 0; i < listAccount.size(); i++) {
			if (!(listAccount.get(i).getCurrency().equalsIgnoreCase("TL"))) {
				newlistAccountName.add(listAccount.get(i).getName());
				listBalance.add(listAccount.get(i).getBalance());

			}
		}
		model.addAttribute("newlistAccount", newlistAccountName);
		...
		if (detail.getDate() != null && detail.getName() != null) {
			...
			if (currentDate.compareTo(enteredDate) <= 0) {
				for (int i = 0; i <listAccount.size(); i++) {
					if (listAccount.get(i).getName().equalsIgnoreCase(detail.getName())&&
						!(listAccount.get(i).getCurrency().equalsIgnoreCase("TL")) ){
						if (detail.getAmount() > 0 && listAccount.get(i).getBalance() >= detail.getAmount()) {
							@SuppressWarnings("deprecation")
							account oldAccountData = accountRepository.getOne(listAccount.get(i).getId());
							oldAccountData.setBalance(listAccount.get(i).getBalance() - detail.getAmount());
							...
							NewAccountActivities.setExpense_details("Döviz Satımı");
							account_activities saveNewAccountActivities = accountActivitiesRepository
									.save(NewAccountActivities);
							detail.setCurrencyType(listAccount.get(i).getCurrency());
							...
							for (int i1 = 0; i1 < listCurrencyValue.size(); i1++) {
								if (listCurrencyValue.get(i1).getCurrency_type()
									...
									float tl_value = detail.getAmount()* newData.getTL_Value();
									message = detail.getAmount() + " " + listAccount.get(i).getCurrency() + " Satım işlemi başarılı. TL karşılığı "+tl_value+" ₺";
									model.addAttribute("message", message);
								}
							}
